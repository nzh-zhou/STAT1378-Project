...
params:
  ...
  dat: zedispensergoeshere # place the name of the csv file here including the extension
  question: 2
---
...
```{r atfig, out.width = "60%", fig.align = "center", fig.pos = "H", fig.cap = paste0("Graphs to test assumptions for question ", params$question)}
if (params$question %in% c(1, 2)) assumptions(params$question, dat)
```

```{r attab}
if (params$question == 3) assumptions(params$question, dat) %>% 
  kbl(booktabs = TRUE, escape = FALSE, digits = 0,
      caption = paste0("Expected value contingency table to test assumptions for question ", params$question)) %>% 
...
Since the statistical test assumptions are satisfied, we can now go ahead with the test. We use functions from the `stats` package in R to conduct the test. Table \@ref(tab:ctt) contains a summary of values obtained from the test.

```{r ctt}
fit(params$question, dat) %>% 
  kbl(col.names = NULL, escape = FALSE, align = "rl", 
      caption = paste0("Summary of the statistical test for question ", 
                       params$question)) %>% 
  kable_styling(latex_options = "hold_position") %>% 
...
Based off the p-value in table \@ref(tab:ctt), `r decision(params$question, dat)`.

This means that `r conclusion(params$question, dat)`.

# Discussion and conclusion

```{r}
decision <- (c(summary(lm(dat$weight ~ dat$height))$coefficients[2, 4],
...
statement <- c(c("there is no linear relationship between height and weight", 
                 ...
                 c("gender and physical activity are independent",
                   "gender and physical activity are dependent")[decision])[params$question]
```
...
If you are reading this, then you must have downloaded and installed my package. I've written the function `mytest` in my package, which does the whole statistical test for a specific question like I have done in this report, and you can use it like below. Run `?project::mytest` for more details.
